const UserDetails = require("../../models/auth/detailUserModel");
...
const userDetailContainer = {
	postUserDetails: async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ message: "Đã xảy ra lỗi khi tạo chi tiết người dùng." });
		}
	},
	...
	getUserDetailsByUserId: async (req, res) => {
		try {
			...
			if (!userDetails) {
				return res
					.status(404)
					.json({ message: "Không tìm thấy chi tiết người dùng." });
			}
			res.status(200).json(userDetails);
		} catch (error) {
			console.error(error);
			res
				.status(500)
				.json({ message: "Đã xảy ra lỗi khi lấy chi tiết người dùng." });
		}
	},
